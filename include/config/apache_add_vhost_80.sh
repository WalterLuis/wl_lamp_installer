...
/bin/sudo /bin/echo "<VirtualHost *:80>" >>/etc/httpd/conf.vhosts.d/${SERVER_NAME}.conf
/bin/sudo /bin/echo "  ServerName        ${SERVER_NAME}:80" >>/etc/httpd/conf.vhosts.d/${SERVER_NAME}.conf
...
/bin/sudo /bin/echo "  ErrorLog          \"logs/${SERVER_NAME}.log\"" >>/etc/httpd/conf.vhosts.d/${SERVER_NAME}.conf
/bin/sudo /bin/echo "  TransferLog       \"logs/${SERVER_NAME}_access.log\"" >>/etc/httpd/conf.vhosts.d/${SERVER_NAME}.conf
if [ "${PARAMS_SSL}" == "443" ]; then
  /bin/sudo /bin/echo "  Redirect          / https://${SERVER_NAME}:443/" >>/etc/httpd/conf.vhosts.d/${SERVER_NAME}.conf
fi
...
#/bin/sudo /bin/echo "    <Limit GET POST HEAD PUT DELETE CONNECT OPTIONS CUSTOM TRACK PATCH COPY LINK UNLINK PURGE LOCK UNLOCK PROPFIND VIEW >" >> /etc/httpd/conf.vhosts.d/${SERVER_NAME}.conf
...
/bin/sudo /bin/sh ${SCRIPT_DIR}/include/config/apache_add_headers.sh "${SCRIPT_DIR}" "${SERVER_NAME}" "80"
